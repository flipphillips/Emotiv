...
class EmoConstants(object):
    def __init__(self):
        ...
        # note that I copy my .h into a nice local place to make it
        # easier to use the Emotiv libraries.
        # but
        # on the mac, they get automatically installed
        # /Applications/EmotivResearch/docs/Examples/Xcode/include
        ...
        self.add('SDK/edkErrorCode.h')
        self.add('SDK/edk.h')
        self.add('SDK/EmoStateDLL.h')
    ...
    def add(self, path):
        ...
        with open(path, 'r') as f:
            for line in f:
                if line.startswith('#define'):
                ...
                elif not enum_mode and \
                        line.lstrip().startswith('typedef enum'):
                    split = line.split()
                    if len(split) < 3:
                        raise Exception(
                            "Unexpected enum formatting line: " +
                            line)
                    enum_mode = True
                    ...
                    for variable in line.split(','):
                        ...
                        if len(split) == 0:
                            ...
                            self.constant_descriptions[split[0]] = \
                                current_enum_description
    ...
    def describe(self, constant, enum_type=None):
        possible_names = [name for name, value in self.constants.items()
                          if value == constant]
        possible_descriptions = [self.constant_descriptions[name]
                                 for name in possible_names]
        description = None
        for (name, name_description) in \
                zip(possible_names, possible_descriptions):
